...
from axon.cli._print import console, ok, warn, fatal, line
...
def ga_use(
    ...
    if context not in cfg.gateways:
        ...
        console.print()
        console.print(line("[dim]available contexts:[/dim]"))
        for name in cfg.gateways:
            marker = " [cyan]← current[/cyan]" if name == cfg.current_gateway else ""
            console.print(line(f"[dim]{name}[/dim]{marker}"))
        console.print()
        console.print(line(f"[dim]create with: axon ga init --name {context} --port <port>[/dim]"))
        console.print()
        raise typer.Exit(1)

    if context == cfg.current_gateway:
        console.print()
        console.print(line(f"[dim]already on context [bold]{context}[/bold][/dim]"))
        console.print()
        raise typer.Exit(0)
    ...
    console.print()
    console.print(line("[dim]operations now target this gateway:[/dim]"))
    console.print(line("[dim]  axon add agent <url>[/dim]"))
    console.print(line("[dim]  axon token generate --name <name>[/dim]"))
    console.print(line("[dim]  axon ga resource list[/dim]"))
    console.print()