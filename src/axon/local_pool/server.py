...
if __name__ == "__main__":
    # stdout is the MCP stdio channel — startup logs MUST go to stderr,
    # otherwise they corrupt the JSON-RPC stream the client reads.
    import os
    import sys
    print(
        f"[local_pool.server] booting MCP server '{mcp.name}' (pid={os.getpid()})",
        file=sys.stderr,
        flush=True,
    )
    mcp.run()